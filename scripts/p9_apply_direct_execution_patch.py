...
def patch_execution() -> None:
    path = Path("src/tiny_qwen_coder/evaluation/execution.py")
    replace_once(
        path,
    ...
    text = path.read_text(encoding="utf-8")
    if "class DirectExecutionHarness(ConstrainedExecutionHarness):" in text:
        raise SystemExit("direct execution harness already exists")
    text += r'''
        ...
        *,
        ...
        with tempfile.TemporaryDirectory(
            ...
            if stdout_thread.is_alive() or stderr_thread.is_alive():
                raise ExecutionHarnessError("direct output capture threads did not terminate cleanly")
...
def patch_rank_evaluator() -> None:
    path = Path("src/tiny_qwen_coder/evaluation/python_rank_sweep.py")
    replace_once(
        path,
    ...
    old_score = '''def score_rank_stage(
    *,
    rank: int,
    baseline_dir: Path,
    repo_root: Path = Path("."),
...
'''
    new_score = '''def score_rank_stage(
    *,
    rank: int,
    baseline_dir: Path,
    repo_root: Path = Path("."),
...
'''
    ...
    old_verify = '''def verify_rank_evaluation(
    *,
    rank: int,
    baseline_dir: Path,
    repo_root: Path = Path("."),
    ...
    output_dir = Path(evaluation.output_dir)
'''
    new_verify = '''def verify_rank_evaluation(
    *,
    rank: int,
    baseline_dir: Path,
    repo_root: Path = Path("."),
...
'''
    ...
    replace_once(
        path,
        "                    baseline_dir=cast(Path, args.baseline_dir),\n                    repo_root=repo_root,\n                ),\n",
        "                    baseline_dir=cast(Path, args.baseline_dir),\n                    repo_root=repo_root,\n                    generation_source_git_sha=cast(\n                        str | None, args.generation_source_git_sha\n                    ),\n                ),\n",
    )


def patch_workflow() -> None:
    path = Path(".github/workflows/python-p9-rank-sweep-evaluation.yml")
    replace_once(
        path,
    ...
    replace_once(
        path,
        "      EXECUTION_IMAGE: python:3.11.14-slim@sha256:c8271b1f627d0068857dce5b53e14a9558603b527e46f1f901722f935b786a39\n",
        "",
    )
    replace_once(
        path,
        '''      - name: Prepare constrained execution image
        run: |
          set -euo pipefail
          docker --version
          docker pull "${EXECUTION_IMAGE}"
          docker image inspect "${EXECUTION_IMAGE}" >/dev/null
...
''',
    ...
    replace_once(
        path,
        "      - name: Score transported responses under constrained execution\n",
        "      - name: Score transported responses inside the runner container\n",
    )


def patch_tests() -> None:
    path = Path("tests/unit/test_constrained_execution.py")
    replace_once(
        path,
        "    ConstrainedExecutionHarness,\n",
        "    ConstrainedExecutionHarness,\n    DirectExecutionHarness,\n",
    ...
    text = path.read_text(encoding="utf-8")
    if "test_direct_execution_requires_explicit_reduced_isolation_opt_in" in text:
        raise SystemExit("direct execution tests already exist")
    text += r'''
